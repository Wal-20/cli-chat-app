...
func NewChatroomRepository(db *gorm.DB) *GormChatroomRepository {
	return &GormChatroomRepository{db: db}
}
...
func (r *GormChatroomRepository) GetPublicChatroomsNotJoined(userID uint) ([]models.Chatroom, error) {
	...
	joinedSub := r.db.Table("user_chatrooms").
		Select("chatroom_id").
		Where("user_id = ? AND is_joined = ?", userID, true)
	// Also exclude rooms where the user is banned
	bannedSub := r.db.Table("user_chatrooms").
		Select("chatroom_id").
		Where("user_id = ? AND is_banned = ?", userID, true)
	err := r.db.Preload("Users").
		Where("id NOT IN (?) AND id NOT IN (?) AND is_public = ?", joinedSub, bannedSub, true).
...
func (r *GormChatroomRepository) FindUserChatroom(userID any, chatroomID any) (*models.UserChatroom, error) {
	...
	if err := r.db.Where("user_id = ? AND chatroom_id = ?", userID, chatroomID).First(&uc).Error; err != nil {
		return nil, err
	}
	return &uc, nil
}

func (r *GormChatroomRepository) CreateUserChatroom(uc *models.UserChatroom) error {
	return r.db.Create(uc).Error
}
func (r *GormChatroomRepository) SaveUserChatroom(uc *models.UserChatroom) error {
...
func (r *GormChatroomRepository) DeleteChatroomByID(id any) error {
	return r.db.Where("id = ?", id).Delete(&models.Chatroom{}).Error
}
...
func (r *GormChatroomRepository) SaveNotification(n *models.Notification) error {
